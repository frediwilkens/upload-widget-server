...
describe('get uploads', () => {
  it('should be able to get uploads', async () => {
    ...
    const systemUnderTest = await getUploads({
      searchQuery: namePattern,
    })

    expect(isRight(systemUnderTest)).toBe(true)
    expect(unwrapEither(systemUnderTest).total).toBe(5)
    expect(unwrapEither(systemUnderTest).uploads).toEqual([
